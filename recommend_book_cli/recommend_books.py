...
def optimize_book_to_buy(
    ...
    """数理最適化を行い、お薦めする書籍のリストを求める"""

    # 既読
    titles_set = df.loc[df["name"] == name, "title"].unique()
    # 未読
    ...
    # お薦めする書籍のタイトルの一覧だけを取得したい場合は、次のように記述する（誤差による不具合を防ぐために0.5を境界として比較する）
...
if __name__ == "__main__":
    ...
    input_file = args.input_dir / "booklist.csv"
    ...
    logger.info("お薦めする書籍を求める処理を開始します。")
    output_df = optimize_book_to_buy(input_df, args.name, 5000)
    logger.debug("結果: \n%s", output_df)

    output_file = args.output_dir / f"result-{args.name}-{args.money}.csv"
